...
const FALLBACK_PRESCRIPTION = {
  rightNow:    "Start with water — even mild dehydration amplifies fatigue and clouds recovery.",
  thisMorning: "Delay caffeine until 10am so cortisol can follow its normal morning curve.",
  today:       "Protect one deep-focus window. Let the rest of the day run shallow.",
  avoid:       "High-intensity training. You'll create more debt, not fitness.",
};
    ...
    <div className="relative min-h-svh flex flex-col px-5 overflow-hidden"
      ...
      >
        ...
        <p className="text-xs mt-1" style={{ color: "var(--color-text-faint)" }}>
          {analysis?.agentTrace?.source === "qvac-local"
            ? "Generated by your self-hosted QVAC agents from the check-in you just logged."
            : "Built from your check-in, your history, and the recovery model."}
        </p>
      </motion.div>
        ...
        {memoryData?.enabled && (memoryData.profile || memoryData.memories.length > 0) && (
          ...
          >
            ...
            <p className="text-[11px] leading-relaxed mb-3" style={{ color: "var(--color-text-secondary)" }}>
              Your coach shaped this from your history:
            </p>
            ...
            <div className="flex items-center gap-1.5 mt-3 pt-2" style={{ borderTop: "1px solid rgba(168,85,247,0.1)" }}>
              <span className="text-[8px] font-mono" style={{ color: "var(--color-text-faint)" }}>
                Your coach considers this before each prescription
              </span>
            </div>
          </motion.div>
        )}
